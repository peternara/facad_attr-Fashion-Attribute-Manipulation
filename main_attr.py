...
experiment_name = os.path.join(experiment_BASE, 'mse_base')
...
train_dataset = datasets.FacadAttrDataset(data_dir, train_img_set, mode='train',
                                        transforms = img_transforms)

test_dataset = datasets.FacadAttrDataset(data_dir, test_img_set, mode='test',
                                        transforms = img_transforms)
...
def train_model(BASE_PATH):
    ...
    for epoch in range(num_epochs):
        for i, data in enumerate(train_dataloader):
            anc_img, anc_attr ,pos_img, pos_attr,_,_ = data
            anc_img, anc_attr, pos_img, pos_attr = anc_img.to(device), anc_attr.to(device),pos_img.to(device), pos_attr.to(device)
            ...
            mse_loss = criterion(anc_emb + attr_emb, pos_emb)
            mse_loss.backward()
            optimizer.step()
        ...
        for img in pred:
            ...
            if np.allclose(img_attr,t_attr):
                if Flag:
                    ...
                    Flag = False
                true_positive += 1
    return correct, total, true_positive, total_precision
